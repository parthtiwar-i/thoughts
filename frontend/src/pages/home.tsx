...
const Home = () => {
  ...
  const hasLoadedRef = useRef<boolean>(false); // Track first-time loading
  ...
  useEffect(() => {
    if (!hasLoadedRef.current) {
      ...
      // Skip loading on subsequent renders
      setLoading(false);
  ...
  useEffect(() => {
    if (loadingComplete && !hasLoadedRef.current) {
      hasLoadedRef.current = true; // Set the flag after first load
      setTimeout(() => {
        setShowContent(true);
        ...
        <>
          ...
          <section className="relative min-h-screen flex items-center justify-center px-4 sm:px-6">
            <div className="text-center max-w-4xl mx-auto">
              ...
              >
                Where every page turn reveals a story from a bygone era,
